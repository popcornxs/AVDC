...
class MyMAinWindow(QMainWindow, Ui_AVDV):
    ...
    logMessage = pyqtSignal(str)  # 日志消息信号量，确保跨线程安全更新 GUI
    ...
    def Init_Ui(self):
        ...
        self.logMessage.connect(self._update_log_ui)
    ...
    def init_config_clicked(self):
        ...
        self.Load_Config()
        self.add_text_main('[+]Config reset to default')

    # ========================================================================加载config
    def Load_Config(self):
        config = ConfigParser()
        config.read(self.get_config_path(), encoding='UTF-8')
        # ========================================================================common
    ...
    def save_config_clicked(self):
        ...
        save_config(json_config)
        self.Load_Config()
        self.add_text_main('[+]Config saved: ' + self.get_config_path())
    ...
    def add_text_main(self, text):
        try:
            ...
            # 通过信号安全地在主线程更新 GUI，避免跨线程操作导致闪退
            self.logMessage.emit(str(text))
        except Exception as error_info:
            print('[-]Error in add_text_main: ' + str(error_info))
    ...
    def _update_log_ui(self, text):
        ...
        try:
            ...
            print('[-]Error in _update_log_ui: ' + str(error_info))
    ...
    def DownloadFileWithFilename(self, url, filename, path, Config, filepath, failed_folder):
        ...
        while i < retry_count:
            try:
                ...
                # 根据 URL 域名添加 Referer 头和 Cookie，绕过反爬和年龄验证
                cookies = {}
                if 'javbus' in str(url):
                    headers['Referer'] = 'https://www.javbus.com/'
                    cookies = {'age': 'verified', 'existmag': 'all'}
                elif 'dmm.co.jp' in str(url):
                    headers['Referer'] = 'https://www.dmm.co.jp/'
                    cookies = {'age_check_done': '1'}
                elif 'javdb' in str(url):
                    headers['Referer'] = 'https://javdb.com/'
                elif 'avsox' in str(url):
                    headers['Referer'] = 'https://avsox.click/'
        ...
        for movie in movie_list:  # 遍历电影列表 交给core处理
            ...
            try:
                ...
                if result != 'not found' and movie_number != '' and result != 'error':
                    ...
                    # 网络超时时跳过当前文件继续处理，不中断整个流程
                    node = QTreeWidgetItem(self.item_fail)
                    node.setText(0, str(self.count_claw) + '-' + str(count) + '.' + os.path.splitext(movie.split('/')[-1])[0])
                    self.item_fail.addChild(node)
                    self.add_text_main('[!]Skipping due to network error, continuing...')
