...
# javbus 年龄验证绕过 cookie
JAVBUS_COOKIES = {'age': 'verified', 'dv': '1'}
...
def getActorPhoto(htmlcode):
    ...
    for i in a:
        l = i.a['href']
        t = i.get_text()
        html = etree.fromstring(get_html(l, cookies=JAVBUS_COOKIES), etree.HTMLParser())
...
def find_number(number):
    ...
    if not (re.match('^\d{4,}', number) or re.match('n\d{4}', number) or 'HEYZO' in number.upper()):
        htmlcode = get_html('https://www.javbus.com/search/' + number + '&type=1', cookies=JAVBUS_COOKIES)
        ...
        if counts != 0:
            for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
                ...
                if number_get == number or number_get == number.replace('-', '') or number_get == number.replace('_',
                    ...
                    return result_url
    # =======================================================================无码搜索
    htmlcode = get_html('https://www.javbus.com/uncensored/search/' + number + '&type=1', cookies=JAVBUS_COOKIES)
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    counts = len(html.xpath("//div[@id='waterfall']/div[@id='waterfall']/div"))
    if counts == 0:
        return 'not found'
...
def main(number, appoint_url):
    try:
        ...
        htmlcode = get_html(result_url, cookies=JAVBUS_COOKIES)
...
def main_us(number, appoint_url=''):
    try:
        if appoint_url:
            ...
            htmlcode = get_html('https://www.javbus.one/search/' + number, cookies=JAVBUS_COOKIES)
            if str(htmlcode) == 'ProxyError':
                raise TimeoutError
            html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
