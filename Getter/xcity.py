...
def find_number(number, appoint_url):
    ...
    # 最多遍历前 10 条结果，避免过多网络请求
    max_count = min(counts + 1, 12)
    if counts >= 2:
        for count in range(2, max_count):  # 遍历搜索结果，找到需要的番号
            href_list = html.xpath("//div[@id='searchResult']/table[@class='resultList']/tr[" + str(count) + "]/td[1]/a/@href")
            if not href_list:
                continue
            result_url = 'https://xcity.jp' + href_list[0]
            detail_page = get_html(result_url)
            detail_page_html = etree.fromstring(detail_page, etree.HTMLParser())
            hinban_list = detail_page_html.xpath("//span[@id='hinban']/text()")
            if not hinban_list:
                continue
            number_get = str(hinban_list[0])
